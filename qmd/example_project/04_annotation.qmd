...
Let's edit the `Annotate Opts` and increase the memory allocation to 60 GB. In this window, you could also edit the Mitos2 and tRNAscan-SE options. 

In the future, we will allow users to specify custom reference databases for annotation. Here we will use the Chordata reference database. However, the Metazoa database generally works well for many taxa.

![Fig. 32](/images/32.png)

You can also edit the `Curate Opts`. For the test dataset, most of the default options are fine, but let's increase the memory to 24 Gb.
...
To run the Annotate module, you can use any of the approaches described in the previous [Nextflow](02_nextflow.qmd) section. Let's use the GUI method.
...
You can filter warnings using the dropdown menu at the lop left. The number in the `warnings` column will update to reflect the selected warning messages. This may be useful for isolating batches of samples that all were flagged with similar warnings.
